...
export default function WithdrawPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!cents) return setErr("Informe um valor válido (ex.: 50,00).");
    ...
    try {
      ...
      if (!res.ok || !json?.ok) return setErr(json?.error ?? "Falha no saque.");
      setOk("Saque realizado!");
      setAmount(""); setNote("");
  ...
  return (
    <main className="relative min-h-screen overflow-hidden">
      ...
      <section className="mx-auto max-w-xl px-6 py-10">
        <Header title="Sacar" subtitle="Retire um valor do seu saldo" color="rose" />
        ...
        <div className="mt-6 rounded-2xl border border-white/50 bg-white/70 p-6 shadow-xl backdrop-blur-md">
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <Field label="Valor (ex.: 50,00)">
              <input
                value={amount}
                onChange={(e)=>setAmount(e.target.value)}
                inputMode="decimal"
                className="w-full rounded-lg border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-rose-600"
                placeholder="50,00"
              />
            </Field>
            <Field label="Observação (opcional)">
              <input
                ...
                className="w-full rounded-lg border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-rose-600"
                placeholder="Ex.: saque em espécie"
              />
            </Field>
            ...
            <div className="flex items-center gap-2 pt-2">
              <button
                disabled={loading}
                className="rounded-lg bg-rose-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:brightness-110 disabled:opacity-60"
              >
...
function Header({ title, subtitle, color }:{title:string; subtitle:string; color:"emerald"|"sky"|"rose"}) {
  const from = color==="rose" ? "from-rose-500" : color==="sky" ? "from-sky-500" : "from-emerald-500";
  const to   = color==="rose" ? "to-pink-400" : color==="sky" ? "to-indigo-400" : "to-teal-400";
  return (
    <div>
...
function Bg() {
  return (
    <div className="pointer-events-none absolute inset-0 -z-10">
      <div className="absolute inset-0 [background:radial-gradient(80rem_60rem_at_50%_-10%,rgb(244_63_94_/_0.14),transparent_50%)]" />
